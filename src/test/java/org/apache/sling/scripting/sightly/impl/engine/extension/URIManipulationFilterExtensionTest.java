...
public class URIManipulationFilterExtensionTest {
    ...
    public void testUnescapePercentInQuery() throws URISyntaxException {
        ...
        assertEquals(
                "http://example.com/example/search.html?q=6%25-10%25#fragment=%25",
        ...
        assertEquals(
                "http://example.com/example/search.html?q=6%25-10%25",
                URIManipulationFilterExtension.unescapePercentInQuery(testUri.toString()));
    }
    ...
    public void testHandleSelectorsStringArrayRemoveSelectors() {
        ...
        final RuntimeObjectModel rom = new AbstractRuntimeObjectModel() {};
        underTest.handleSelectors(
                rom, builder, Collections.singletonMap("removeSelectors", new String[] {"a", "b", "s1"}));
        assertEquals("s2", builder.getSelectorString());
    }
}
